...
void jouer(SDL_Surface* ecran){
 ...
 TTF_Font *police;
  ...
  Background2 = SDL_LoadBMP("images/backgroud2.bmp");
  surface_perdu = SDL_LoadBMP("images/perdue.bmp");
  surface_aide = SDL_LoadBMP("images/key.bmp");
if( (Background2 == NULL ) || (surface_perdu == NULL ) || ( surface_aide == NULL ) ){ 
  ...
  while (continuer){
    ...
    if(!jeu_pas_fini){
 SDL_SetColorKey(surface_perdu , SDL_SRCCOLORKEY, SDL_MapRGB( surface_perdu ->format, 255,255,255));
...
void tourner_piece (int l ,int h ,int  matrice_jeu[l][h]){
  ...
  for (i=22;i>=0 && !tetramino_trouve;i--){
	...
	case 1:
	  switch (rotation) { 
	  case 0: //On vérifie les cases a remplir
